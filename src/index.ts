...
export class MAL {
    ...
    > {
        ...
        const [data, err] = await MAL.get<_MALstandardListResponse<{ node: Anime }>>(`${uri}`, reqOption);
        if (err == undefined) {
    ...
    > {
        const reqOption: RequestInit = this.genenrateHeader();
        const uri = `${this.url}/anime/${animeId}?fields=${[fields ?? AllAnimeDetailFields].join(",")}`;
        if (!fields)
    ...
    public async getSeasonalAnime(option: {
        ...
        offset?: number
        fields: AnimeFields[]
    }): Promise<[(Partial<Anime> & Pick<Anime, "id" | "title">)[], undefined] | [undefined, MalError]>
    public async getSeasonalAnime({ year, season, sort, limit, fields, offset }: {
        year: number,
        season: quarter,
        sort?: "anime_score" | "anime_num_list_users"
        limit?: number
        ...
        const reqOption = this.genenrateHeader()
        let uri = buildURI(this.url + `/anime/season/${year}/${season}`, {
            sort, limit, offset, fields: fields ? fields.join(",") : AllAnimeFields.join(",")
        })
        const [data, err] = await MAL.get<_MALstandardListResponse<{ node: Anime }>>(uri, reqOption);
